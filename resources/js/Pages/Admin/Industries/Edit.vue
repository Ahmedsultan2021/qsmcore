...
const selectedIds = computed(() =>
    (props.industry.form_templates || []).map((ft) => ft.id)
);

const form = useForm({
    name: props.industry.name || "",
    description: props.industry.description || "",
    form_template_ids: selectedIds.value,
});
    ...
    <div class="p-6">
        <BaseDashboardHeader
            :navs="[
                ...
                { name: industry.name, linkName: 'industries.show', param: industry.id },
                { name: 'Edit', linkName: 'industries.edit', param: industry.id },
            ]"
            title="Edit Industry"
            :showButton="false"
