...
const submit = () => {
    form.post(route("companies.login"), {
...
<template>
    <Head title="Companies Portal - Login" />

    <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8">
            <div>
                <div class="flex justify-center">
                    <img src="/logos/logo-width.png" class="h-16 w-auto" alt="QSM Logo" />
                </div>
            ...
            <form class="mt-8 space-y-6" @submit.prevent="submit">
                ...
                <div v-if="form.errors.email" class="text-sm text-red-600 text-center">
                    {{ form.errors.email }}
                </div>

                <div class="text-center">
                    <Link :href="route('welcome')" class="text-sm text-blue-600 hover:text-blue-500">
                        ← Back to Home
                    </Link>
                </div>
            </form>
